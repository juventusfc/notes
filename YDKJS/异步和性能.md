...
```javascript
let fetchResponse = new Promise(function(resolve, reject) { //在这种情况下，传入的函数会立即执行。
...
最后一个例子,`then(callback1,callback2)`相当于是对Promise的监听，Promise完成后，执行回调callback1或callback2。

```javascript
p.then(function(){ // 第一个p.then
    p.then(function(){ // 第三个p.then
        console.log("C");
    });
    console.log("A");
});
p.then(function(){ // 第二个p.then
    console.log("B");
});
// 执行次序：
...
* 注意事项
  ...
  下面的p是立即完成的Promise，但是对一个Promise调用`then()`时，即使这个Promise已经完成，`then()`中的回调也是**异步**的。
